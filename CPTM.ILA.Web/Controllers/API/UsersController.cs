...
namespace CPTM.ILA.Web.Controllers.API
{
    ...
    public class UsersController : ApiController
    {
        ...
        {
            if (user.Username.ToUpper() == "LGPDCOMUM")
            {
                var areaTratamentoDados = new AgenteTratamento()
                {
                    Nome = "LGPDCOMUM",
                    Area = "LGPDTESTE",
                    Email = "lgpdcomum@test.com.br",
                    Telefone = "+0000000000"
                };
                ...
                if (userInDb == null)
                {
                    ...
                    var userOriginGroup =
                        await _context.Groups.SingleOrDefaultAsync(g => g.Nome.ToUpper() == "LGPDTESTE") ??
                        new Group()
                        {
                            Nome = "LGPDTESTE",
                        };
                ...
                var jwtToken = TokenUtil.CreateToken(userInDb, new UsuarioAD() { Nome = "LGPDCOMUM" }, false);
